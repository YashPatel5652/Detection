...
class Phase1Sender:
    def __init__(self, args):
        ...
        self.logger.info(f"Phase 1 Sender initialized")
    ...
    def calculate_packet_sizing(self):
        ...
        self.raw_chunk_size = 1440  # Conservative default
        ...
        # Safety margin (reduce by ~5% to account for JSON & headers)
        self.raw_chunk_size = int(self.raw_chunk_size * 0.95)
    ...
    def create_metadata_packet(self, file_id: str, metadata_path: str) -> List[str]:
        ...
        try:
            ...
            packets = []
            max_content_size = self.args.packet_size - len(header.encode('utf-8')) - len(DELIMITER) - 20
    ...
    def send_packet_with_retry(self, sock: socket.socket, packet: str, expected_ack: str) -> bool:
        """Send packet and wait for ACK with retry logic."""
        for attempt in range(self.args.max_retries + 1):
            try:
                if self.args.verbose:
                    self.logger.debug(f"Sending packet (attempt {attempt + 1}): {packet[:100]}...")
                
                # Send packet
                packet_bytes = packet.encode('utf-8')
                sock.sendall(len(packet_bytes).to_bytes(4, 'big'))  # Send length first
                sock.sendall(packet_bytes)
                
                # Wait for ACK
                ack_len_bytes = sock.recv(4)
                if len(ack_len_bytes) != 4:
                    raise ConnectionError("Failed to receive ACK length")
                
                ack_len = int.from_bytes(ack_len_bytes, 'big')
                ack_data = sock.recv(ack_len).decode('utf-8')
                
                if self.args.verbose:
                    self.logger.debug(f"Received ACK: {ack_data[:100]}...")
                
                # Parse ACK
                if self.parse_ack_response(ack_data, expected_ack):
                    return True
                else:
                    self.logger.warning(f"Received NACK or error for {expected_ack}")
    ...
    def send_file(self, file_info: FileInfo) -> bool:
        ...
        try:
            self.logger.info(f"Starting transfer of {file_info.filename}")
            
            with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
                sock.settimeout(60)  # Increased timeout for FILE_COMPLETE
                ...
                if not self.send_packet_with_retry(sock, init_packet, f"INIT {file_info.file_id}"):
                ...
                for i, metadata_packet in enumerate(metadata_packets):
                    if not self.send_packet_with_retry(sock, metadata_packet, f"METADATA {file_info.file_id}"):
                ...
                for part_index in range(file_info.total_parts):
                    ...
                    if self.send_packet_with_retry(sock, part_packet, f"PART {file_info.file_id} {part_index+1}"):
                ...
                # Wait for FILE_COMPLETE - FIXED PARSING
                self.logger.info(f"Waiting for FILE_COMPLETE confirmation...")
                
                # Receive FILE_COMPLETE response
                complete_len_bytes = sock.recv(4)
                if len(complete_len_bytes) != 4:
                    raise ConnectionError("Failed to receive FILE_COMPLETE length")
                
                complete_len = int.from_bytes(complete_len_bytes, 'big')
                complete_data_bytes = b""
                while len(complete_data_bytes) < complete_len:
                    chunk = sock.recv(complete_len - len(complete_data_bytes))
                    if not chunk:
                        raise ConnectionError("Connection closed during FILE_COMPLETE reception")
                    complete_data_bytes += chunk
                
                complete_data = complete_data_bytes.decode('utf-8')
                
                if self.args.verbose:
                    self.logger.debug(f"Received FILE_COMPLETE: {complete_data}")
                
                if self.parse_file_complete_response(complete_data, file_info):
                    ...
                    raise RuntimeError("FILE_COMPLETE confirmation failed")
    ...
    def parse_file_complete_response(self, response_data: str, file_info: FileInfo) -> bool:
        """Parse FILE_COMPLETE response - FIXED VERSION."""
        try:
            if self.args.verbose:
    ...
    def run(self):
        ...
        self.logger.info("Starting Phase 1 Sender daemon")
        ...
        try:
            ...
            self.running = False
            self.logger.info("Phase 1 Sender daemon stopped")


def main():
    parser = argparse.ArgumentParser(description="Phase 1 Sender - Transfer anomaly detection data to Phase 2")
    ...
    parser.add_argument('--max-retries', type=int, default=5,
                       help='Maximum retry attempts per packet (default: 5)')
